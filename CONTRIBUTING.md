...
Перед выпуском в
[CHANGELOG.md](https://github.com/andrey-aka-skif/AndreyAkaSkif.ServiceDefaults/blob/master/CHANGELOG.md)
добавляется секция версии — коммитом `chore(release): зафиксирована версия X.Y.Z`
и до создания релиза, чтобы тег указывал на состояние с готовым журналом. Журнал
ведётся с 0.0.46, заметки к более ранним выпускам остались в Releases.
...
.NET в образ раннера не входит и целиком ставится `setup-dotnet`, ровно как на
GitHub-стороне. Плата за это — SDK обеих версий качается каждый прогон, и
раннеру нужен исходящий доступ к серверам Microsoft. Снимается собственным
образом (`sdk` + `nodejs` + рантайм 9), но его место — рядом с конфигурацией
раннера, а не в этом репозитории: образ общий для всех .NET-проектов.
