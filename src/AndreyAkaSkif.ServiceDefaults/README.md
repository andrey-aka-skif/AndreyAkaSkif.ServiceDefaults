...
`Microsoft.AspNetCore.Mvc.Json` для контроллеров. Генератор спецификации читает те же
параметры, что и конвейер ответов, а какой из двух наборов — зависит от генератора.
Приложению без контроллеров это не вредит.
...
в списке серверов (см. `Servers` в README пакета `AndreyAkaSkif.ServiceDefaults.OpenApi`).
...
и не конфигурируется: то же значение по умолчанию берёт `AddHealthCheckEndpointDescription()`
из пакета `AndreyAkaSkif.ServiceDefaults.OpenApi`, который описывает эту конечную точку
в спецификации.
