...
public static class StringEnumJsonExtensions
{
    ...
    /// Генератор спецификации читает те же параметры сериализации, что и конвейер ответов,
    /// — какой из двух наборов, зависит от генератора, — поэтому настроены оба: иначе тела
    /// ответов содержали бы имена, а спецификация по-прежнему объявляла бы перечисление
    /// целочисленным. Приложению без контроллеров вторая настройка не вредит.
    /// </para>
    /// <para>
